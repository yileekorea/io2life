...
$data_4_room = new USER();

if(!$data_4_room->is_logged_in())
{
	$data_4_room->redirect('index.php');
}
...
//+--------------+---------------+------+-----+---------+----------------+
$tbl='tbl_users';
$stmt = $data_4_room->runQuery("SELECT * FROM $tbl WHERE userID=:uid");
...
//+-----------------+------------------+------+-----+---------+-------+
$tbl = $row['tbl_TempSet'];
$stmt = $data_4_room->runQuery("SELECT * FROM $tbl");
$stmt->execute();
...
for ($x = 0; $x < ($tbl_TempSet[0]['numSensor']); $x++) {
	if (isset($_GET["dateParam"])) {
		$stmt = $data_4_room->runQuery("SELECT timestamp_value, current_temps, current_status FROM $tbl WHERE id=$x+1 AND timestamp_value LIKE '".$_GET["dateParam"]."%'");
		//$stmt = $data_4_room->runQuery("SELECT timestamp_value, current_temps, current_status FROM $tbl WHERE id=$x+1 AND timestamp_value LIKE '".date('Y-m-d')."%'");
	} else {
		//$stmt = $data_4_room->runQuery("SELECT timestamp_value, current_temps, current_status FROM $tbl WHERE id=$x+1 AND timestamp_value LIKE '".date('Y-m-d')."%'");
		$stmt = $data_4_room->runQuery("SELECT timestamp_value, current_temps, current_status FROM $tbl WHERE id=$x+1 AND timestamp_value > (now() - interval 1 day)");
	}
	$stmt->execute();
...
*/
		if(($x+1) != $tbl_TempSet[0]['numSensor']){ //마지막 센서는 들어온 물온도 기록
			$row_a[$x]['data'][] = array($strTime, $tbl_RoomTemp['current_status']);
		} else {
			$row_a[$x]['data'][] = array($strTime, $tbl_RoomTemp['current_temps']);	//마지막 센서는 들어온 물온도 기록
		}
